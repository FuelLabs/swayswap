...
import { Input, Menu } from "~/systems/UI";
import type { Coin } from "~/types";

const style = {
  input: `appearance-none w-full rounded-lg bg-gray-700 px-4 py-2 focus-ring text-gray-100`,
...
export function CoinsListDialog({ onSelect }: CoinListModalProps) {
  ...
  const filtered = TOKENS.filter(filterByValid).filter(filterBySearch);
